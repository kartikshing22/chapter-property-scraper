...
class ScraperAPIClient:
    ...
    def account(self):
        return requests.get(
            "https://api.scraperapi.com/account", params={"api_key": self.api_key}
        ).json()
...
def makeRequestWithMethod(
    ...
    query_filtered = {k: v for k, v in query.items() if v is not None}
    # @todo add headers
    r = requests_retry_session(retries=retry).request(
        method,
        "https://api.scraperapi.com/",
        params=query_filtered,
        timeout=timeout,
        data=body,
        headers=headers,
    )
